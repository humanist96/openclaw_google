...
#### 이미지 갤러리 (img-gallery)
- 클릭하여 이미지 업로드 (input[type="file"] multiple)
- 여러 장 이어서 추가 가능
- 각 이미지에 캡션 입력 필드
- 개별 이미지 삭제 버튼 (X)
- 이미지 없을 때: 점선 테두리 플레이스홀더 (카메라 아이콘 + 설명)
- 이미지 있을 때: 축소된 "+" 추가 버튼으로 전환
- localStorage에 이미지 데이터 저장/복원
...
| 이미지 업로드 | `FileReader` + base64 + 여러 장 추가 |
| 이미지 저장/복원 | `localStorage` 기반 영속 저장 |
| 이미지 삭제 | 개별 삭제 + 애니메이션 |
| 캡션 편집 | 인라인 input + 자동 저장 |
